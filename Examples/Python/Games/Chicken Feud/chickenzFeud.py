...
class Turret(pygame.sprite.Sprite):
	def __init__(self):
		...
		self.rect.x = 240; self.rect.y = 560
...
## Set up method to enable the player's turret to move
	...
	def moveKnob(self,xpos):
		self.rect.x = (xpos-320)/3.5
		self.oldpos = xpos
...
class Bullet(pygame.sprite.Sprite):
	def __init__(self,turret):
		...
		self.rect.y=turret.rect.y-turret.rect.height +80
		self.kind = "bullet";
...
## Set up method to update score and remove fruit when shot
	...
	def shot(self,game):
		if self.genus==1:
			...
			game.score+=10
		self.kill()
		os.system('aplay chicken-hit.wav &')
...
pygame.display.set_caption('Chickenz Battle Feud 2000')
os.system('aplay TAKEDOWN.WAV &')
...
while end_game!=True:
	clock.tick(6)   # Adjust Game Speed
	tock+=1
	screen.fill(black)
...
## Move objects
	...
	turret.moveKnob( analog.read(0,0) *2.2 ) # Moves chicken turret using analog to digital converter
...
## Update player score
	...
	statusFont = pygame.font.Font(None,26)
